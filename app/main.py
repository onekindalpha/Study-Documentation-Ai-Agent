...
def normalize_seed_url_for_collection(url: str) -> str:
    """Normalize common pasted learning URLs before collection.
    ...
    if host == "microsoft.github.io" and path == "/agent-academy/video":
        return urlunparse(parsed._replace(path="/agent-academy/videos/"))
    return raw
...
def is_youtube_playlist_only(url: str) -> bool:
    return is_youtube_host(url) and bool(seed_playlist_id(url)) and not youtube_video_id(url)


def clean_youtube_transcript_text(text: str) -> str:
    """Clean transcript text from YouTube caption formats without over-summarizing."""
    raw = html_lib.unescape(str(text or ""))
    raw = re.sub(r"<[^>]+>", " ", raw)
    raw = raw.replace("\ufeff", "")
    raw = re.sub(r"\[Music\]|\[Applause\]|\[음악\]|\[박수\]", " ", raw, flags=re.IGNORECASE)
    lines: list[str] = []
    previous = ""
    for line in raw.splitlines():
        ...
        if "-->" in line:
            continue
        if re.match(r"^(Kind|Language|NOTE|STYLE|REGION)\b", line, re.I):
            continue
        ...
        if not line:
            continue
        # YouTube auto-caption VTT can repeat the same line with sliding windows.
        if line == previous:
            continue
        lines.append(line)
        previous = line
...
def ytdlp_video_metadata(url: str) -> tuple[dict[str, Any], str]:
    try:
        proc = subprocess.run(
            [os.sys.executable, "-m", "yt_dlp", "--dump-json", "--skip-download", url],
            text=True,
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
            timeout=60,
        ...
        if proc.returncode == 0 and proc.stdout.strip():
            return json.loads(proc.stdout.splitlines()[-1]), "yt-dlp metadata success"
        return {}, f"yt-dlp metadata failed: code={proc.returncode}; {(proc.stderr or '')[-1000:]}"
    except Exception as exc:
...
def youtube_transcript_chars_from_source_text(text: str) -> int:
    ...
    # Stop before the next major internal block if one is appended later.
...
def fetch_youtube_source_text(url: str, max_chars: int = 12000) -> str:
    ...
    try:
        ...
        transcript_statuses.append(f"oEmbed metadata failed: {type(exc).__name__}: {exc}")

    metadata, meta_status = ytdlp_video_metadata(url)
    transcript_statuses.append(meta_status)
    if isinstance(metadata, dict) and metadata:
        title = title or str(metadata.get("title") or "").strip()
        author = author or str(metadata.get("uploader") or metadata.get("channel") or "").strip()
    ...
    # youtube-transcript-api but still expose subtitle tracks through yt-dlp.
    if not transcript:
        transcript, status = transcript_from_ytdlp_subtitles(url, video_id=video_id)
        transcript_statuses.append(status)
    ...
    if not transcript:
        try:
            ...
            if isinstance(player, dict):
                ...
                description = description or str(details.get("shortDescription") or "").strip()
                tracks = (
                    player.get("captions", {})
                ...
                if tracks:
                    ...
                    if base_url:
                        caption_text, _ = fetch_url_text(base_url, timeout=12, limit=900000)
                        if "<text" in caption_text:
                            parts = re.findall(r"<text[^>]*>(.*?)</text>", caption_text, flags=re.DOTALL)
                            transcript = "\n".join(clean_youtube_transcript_text(part) for part in parts)
                        else:
                            ...
                            transcript = clean_youtube_transcript_text("".join(chunks))
                        transcript_statuses.append(f"ytInitialPlayerResponse caption success: {track.get('languageCode') or 'caption'}")
        except Exception as exc:
            transcript_statuses.append(f"ytInitialPlayerResponse caption failed: {type(exc).__name__}: {exc}")

    transcript = clean_youtube_transcript_text(transcript)
    ...
    if description:
        lines.append("\n[영상 설명]")
        lines.append(description[:8000])
    lines.append("\n[자막 상태]")
    lines.extend(f"- {status}" for status in transcript_statuses if status)
    if transcript:
        lines.append("\n[영상 자막]")
        lines.append(transcript[:max_chars])
    else:
        lines.append("\n[영상 자막 없음]")
        lines.append("이 영상은 현재 자동 자막 수집 경로에서 transcript를 확보하지 못했습니다. 이 경우 제목/설명만으로 Medium 글을 만들지 않고 중단해야 합니다.")
    return "\n".join(lines)
...
## 다음 단계
1. 이 URL을 source-pack collector로 먼저 수집합니다.
2. 로그인/권한이 필요하면 브라우저에서 사용자가 직접 로그인합니다.
3. 커리큘럼, Summary/Transcript, 영상 후보, Lab/Exercise 링크를 수집합니다.
4. 생성된 source_pack.md 내용을 앱 입력칸에 넣고 Medium 글을 생성합니다.
...
def run_source_pack_collector(url: str, timeout_seconds: int = 420, run_id: str = "") -> tuple[str, dict[str, Any]]:
    ...
    if is_youtube_host(url) and not is_youtube_playlist_only(url):
        ...
        started = time.perf_counter()
        text = fetch_youtube_source_text(url, max_chars=90000)
        transcript_chars = youtube_transcript_chars_from_source_text(text)
        title_match = re.search(r"^영상 제목:\s*(.+)$", text, flags=re.M)
        title = title_match.group(1).strip() if title_match else (f"YouTube video {youtube_video_id(url)}" if youtube_video_id(url) else url)
        quality_status = "pass" if transcript_chars >= 1200 else "fail"
        quality = {
            "source_type": "youtube",
            "quality_status": quality_status,
            "can_generate_article": transcript_chars >= 1200,
            "transcript_chars": transcript_chars,
            "transcript_segments": len([line for line in text.splitlines() if re.match(r"^\[\d{2}:\d{2}\]", line.strip())]),
            "text_chars": len(text),
            "missing": [] if transcript_chars >= 1200 else ["video transcript not accessible"],
        }
        stats = {
            "page_count": 1,
        ...
        payload = {
            ...
            "video_index": [{"title": title, "youtube_url": url, "transcript_chars": transcript_chars}],
            "video_url_candidates": [url],
        ...
        json_path.write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8")
        return text, {
            "ok": transcript_chars >= 1200,
            "collector": "direct_youtube_transcript",
            "command": ["direct_youtube_transcript", url],
            "markdown_path": str(md_path),
            "json_path": str(json_path),
            "quality": quality,
            "stats": stats,
            "stdout": "",
            "stderr": "" if transcript_chars >= 1200 else "video transcript not accessible",
...
def seed_playlist_id(seed_url: str) -> str:
    ...
    return (qs.get("list") or qs.get("playlistId") or qs.get("playlistid") or [""])[0]
...
def source_pack_quality_sufficient(seed_url: str, source_pack_text: str, collector_report: dict[str, Any]) -> tuple[bool, list[str]]:
    ...
    if "youtube.com" in host or "youtu.be" in host:
        transcript_chars = int(quality.get("transcript_chars") or youtube_transcript_chars_from_source_text(source_pack_text) or 0)
        if transcript_chars < 1200:
            reasons.append("video transcript not accessible")
...
def collector_execution_failure_report(seed_url: str, run_id: str, collector_report: dict[str, Any]) -> str:
    ...
    if seed_playlist_id(seed_url):
        return f"""# YouTube 플레이리스트는 아직 자동 글 생성 대상이 아닙니다

입력한 URL은 단일 YouTube 영상이 아니라 플레이리스트 URL입니다.
현재 버전은 플레이리스트 안의 여러 영상을 자동으로 펼쳐서 각 영상 자막을 수집하는 기능까지는 안정화되어 있지 않습니다.

## 입력 URL
- {seed_url}

## 중단 이유
- 플레이리스트 URL에서는 단일 video id를 안정적으로 추출할 수 없습니다.
- 이 상태에서 글을 만들면 제목이나 이전 수집 결과를 근거로 한 부정확한 글이 생성될 수 있어 중단했습니다.
...
이번 URL에서 Medium 글을 만들 만큼 신뢰할 수 있는 본문 또는 자막을 수집하지 못했습니다. 제목이나 URL만 보고 글을 만들면 이전 결과나 일반 키워드가 섞일 수 있으므로 생성을 중단했습니다.
...
## 다음 조치
- 단일 강의/영상 URL인지 확인하세요.
- 로그인, 권한, 동적 렌더링, 자막 비공개 여부 때문에 수집이 막힐 수 있습니다.
- 같은 주제를 계속 테스트하려면 이전 입력창 URL을 지우고 새 URL 하나만 넣는 것이 안전합니다.
"""
...
입력한 URL은 단일 YouTube 영상이 아니라 플레이리스트 URL입니다. 현재 버전은 플레이리스트 내부 영상을 자동으로 펼쳐서 각 영상 자막을 모으는 기능이 안정화되어 있지 않습니다.
...
이번 URL에서 Medium 글을 만들 만큼 충분한 본문, 하위 페이지, 영상 자막, 실습 정보를 수집하지 못했습니다. 이 상태에서 글을 만들면 제목이나 일반 키워드만 보고 추상적인 글이 만들어질 수 있어 차단했습니다.
...
## 다음 조치
- 단일 영상 URL이라면 자막이 공개되어 있는지 확인하세요.
- 강의 목록/플레이리스트라면 개별 강의 URL을 넣어주세요.
- 웹 문서라면 대표 URL이 아니라 실제 본문이 있는 세부 페이지 URL을 넣어주세요.
"""


def seed_mismatch_report(seed_url: str, run_id: str, collector_report: dict[str, Any], reason: str) -> str:
    return f"""# 입력 URL과 수집 결과가 맞지 않아 중단했습니다

이번 생성에서 입력한 URL과 실제 수집된 내용이 서로 맞지 않는 것으로 판단했습니다. 이전에 입력한 URL이나 브라우저 상태가 섞였을 가능성이 있어 글 생성을 차단했습니다.
...
## 다음 조치
- 입력창에 이전 URL이 남아 있다면 모두 지우고 새 URL 하나만 넣어 다시 실행하세요.
- 같은 브라우저 탭에서 계속 테스트 중이면 페이지를 새로고침한 뒤 다시 시도하세요.
"""


def final_article_mismatch_report(seed_url: str, run_id: str, collector_report: dict[str, Any], reason: str, article: str) -> str:
    preview = (article or "").strip()[:1200]
    return f"""# 생성 결과가 입력 URL과 맞지 않아 차단했습니다

수집 이후 글 생성 단계에서 입력 URL과 다른 주제의 글이 만들어진 것으로 판단했습니다. 이 상태의 글은 사용자에게 보여주면 안 되므로 차단했습니다.
...
def article_matches_seed_url(seed_url: str, article: str) -> tuple[bool, str]:
    host = url_domain(seed_url)
...
def youtube_domain_terms(title: str, transcript_text: str, limit: int = 18) -> list[str]:
    ...
    banks: dict[str, list[tuple[str, list[str]]]] = {
        ...
        "vector_retrieval": [
            ...
            ("의미 기반 검색", ["semantic search", "semantic", "의미 검색"]),
            ("하이브리드 검색", ["hybrid retrieval", "hybrid search", "hybrid", "하이브리드"]),
            ("RAG", ["rag", "retrieval augmented", "retrieval-augmented"]),
            ("검색 인덱스", ["index", "indexing", "인덱스"]),
            ("랭킹", ["ranking", "reranking", "rank"]),
            ("쿼리", ["query", "queries", "쿼리"]),
        ...
        "llm_engineering": [
            ...
            ("LLM 애플리케이션", ["llm application", "llm applications", "application"]),
            ("RAG", ["rag", "retrieval augmented", "retrieval-augmented"]),
            ("에이전트", ["agent", "agents", "agentic"]),
            ("평가", ["evaluation", "evaluate", "evals"]),
            ("모니터링", ["monitoring", "monitor"]),
            ("배포", ["deploy", "deployment", "production"]),
            ("프로젝트", ["project", "capstone"]),
            ("워크숍/모듈", ["module", "workshop", "course"]),
        ],
        "copilot_cowork": [
            ("Copilot Cowork", ["cowork", "co-work", "copilot co"]),
            ("클라우드 네이티브 에이전트", ["cloud-native", "running in the cloud", "cloud"]),
            ("비동기 작업 흐름", ["work continues", "step away", "mobile app", "asynchronous"]),
            ("VS Code 연동", ["vs code", "vscode"]),
            ("샌드박스 실행", ["sandbox", "sandboxed"]),
            ("GitHub SDK", ["github sdk", "github"]),
            ("작업 충돌 관리", ["conflict", "conflicts"]),
            ("MCP/플러그인 확장", ["mcp", "plugin", "plugins", "skills"]),
        ],
        "git_github": [
            ("Git", ["git", "깃"]),
            ("GitHub", ["github", "깃허브"]),
            ("버전 관리", ["version control", "버전 관리"]),
            ("commit", ["commit", "커밋"]),
            ("branch", ["branch", "브랜치"]),
            ("merge", ["merge", "머지", "병합"]),
            ("repository", ["repository", "저장소", "repo", "레포"]),
            ("remote", ["remote", "원격"]),
        ...
        "data_structure_algorithm": [
            ...
            ("공간 복잡도", ["공간 복잡도", "space complexity"]),
            ("빅오", ["big o", "big-o", "빅오", "표기법"]),
            ("배열", ["배열", "array"]),
            ("인덱스", ["인덱스", "index"]),
            ("탐색", ["탐색", "search"]),
            ("삽입", ["삽입", "insert"]),
    ...
    bank = banks.get(profile, [])
    found: list[str] = []
    for label, aliases in bank:
...
def youtube_section_title_valid(title: str) -> bool:
    s = re.sub(r"\s+", " ", str(title or "")).strip()
    if not s or len(s) < 2:
        return False
    lower = s.lower()
    bad_starts = (
        "um ", "uh ", "and ", "but ", "so ", "then ", "this is", "here ", "hi ",
        "i want", "we are", "what we", "you can", "if you", "now ", "first thing",
    )
    if lower.startswith(bad_starts):
        return False
    if re.search(r"\b(um|uh|actually|briefly)\b", lower) and len(s) < 80:
        return False
    if len(s.split()) > 9 and not re.search(r"[가-힣]", s):
        return False
    return True


def youtube_chapter_sections(transcript_text: str, limit: int = 6, title: str = "") -> list[dict[str, Any]]:
    sentences = split_learning_sentences(transcript_text, limit=360)
    intro_noise = [
        "얄팍한 코딩 사전", "유튜브 채널", "컨텐츠를 만들고", "온라인 장편 강의",
        "고정댓글", "무료 파트", "구독", "좋아요", "welcome to this event",
        "today we are starting", "watching this video", "live chat", "below the video",
        "i want to show you", "i'm just recording", "previous workshop",
    ]
    sentences = [s for s in sentences if not any(noise in s.lower() for noise in intro_noise)]
    if not sentences:
        return []
    terms = youtube_domain_terms(title, transcript_text, limit=30) or infer_learning_terms(f"{title}\n{transcript_text}", limit=30)
    chunk_size = max(6, min(14, len(sentences) // max(3, min(limit, 6)) + 1))
    ...
    for start in range(0, len(sentences), chunk_size):
        ...
        if not chunk:
            continue
        blob = " ".join(chunk).lower()
        local_terms = [term for term in terms if term.lower() in blob]
        if local_terms:
            title_candidate = " · ".join(local_terms[:3])
        else:
            title_candidate = f"핵심 흐름 {len(sections) + 1}"
        if not youtube_section_title_valid(title_candidate):
            title_candidate = f"핵심 흐름 {len(sections) + 1}"
        key = title_candidate.lower()
        if key in used_titles:
            title_candidate = f"핵심 흐름 {len(sections) + 1}"
            key = title_candidate.lower()
        used_titles.add(key)
        sections.append({"title": title_candidate, "sentences": chunk[:5]})
...
def infer_learning_terms(text: str, limit: int = 16) -> list[str]:
    source = str(text or "")
    lower = source.lower()
    title_hint = source.splitlines()[0].lower() if source.splitlines() else lower[:180]
    profile = youtube_topic_profile(title_hint, source)
    if profile != "general_lecture":
        return youtube_domain_terms(title_hint, source, limit=limit)

    candidates = [
        "시간 복잡도", "공간 복잡도", "빅오", "배열", "인덱스", "탐색", "삽입", "삭제",
        "스택", "큐", "연결 리스트", "트리", "이진 트리", "순회", "그래프", "정렬",
        "프로세스", "스레드", "스케줄링", "메모리", "TCP", "UDP", "HTTP", "트랜잭션", "ACID", "조인",
        "벡터 데이터베이스", "임베딩", "의미 기반 검색", "하이브리드 검색", "RAG", "검색 인덱스", "랭킹", "쿼리",
        "Copilot Studio", "declarative agent", "custom agent", "Adaptive Cards", "Agent Flows",
        "Foundry", "VS Code", "에이전트", "모니터링", "배포", "평가",
    ]
    alias_map = {
        "벡터 데이터베이스": ["vector database", "vector databases"],
        "임베딩": ["embedding", "embeddings"],
        "의미 기반 검색": ["semantic search"],
        "하이브리드 검색": ["hybrid search", "hybrid retrieval"],
        "검색 인덱스": ["indexing", "search index"],
        "에이전트": ["agent", "agents", "agentic"],
    }
    found: list[str] = []
    for term in candidates:
        aliases = [term.lower()] + [a.lower() for a in alias_map.get(term, [])]
        if any(alias in lower for alias in aliases):
            found.append(term)
    git_context = any(x in title_hint for x in ["git", "github", "깃", "깃허브"])
    if git_context:
        for term in ["Git", "GitHub", "버전 관리", "commit", "branch", "merge", "repository", "remote", "Pull Request", "conflict"]:
            if term.lower() in lower or term in ["Git", "GitHub", "버전 관리"]:
                found.append(term)
    rest_context = any(x in title_hint for x in ["rest", "api", "http"])
    if rest_context and ("rest" in lower or "api" in lower or "http" in lower):
        found.append("REST")
    return unique_preserve_order(found, limit=limit)


def youtube_learning_terms(title: str, transcript_text: str, limit: int = 18) -> list[str]:
    return youtube_domain_terms(title, transcript_text, limit=limit) or infer_learning_terms(f"{title}\n{transcript_text}", limit=limit)
...
def youtube_keyword_sections(title: str, transcript_text: str, limit: int = 7) -> list[dict[str, Any]]:
    ...
    sentences = split_learning_sentences(transcript_text, limit=360)
    plan = [
        ("Git이 해결하는 버전 관리 문제", ["git", "깃", "버전", "변경", "되돌아"]),
        ("GitHub와 원격 저장소", ["github", "깃허브", "원격", "저장소", "레포", "repository", "remote"]),
        ("commit으로 변경 이력 남기기", ["commit", "커밋", "변경 사항", "기록", "이력"]),
        ("branch로 작업 흐름 분리", ["branch", "브랜치", "가지", "분기", "작업"]),
        ("merge와 충돌 해결", ["merge", "머지", "병합", "충돌", "conflict"]),
        ("push/pull/clone으로 협업 흐름 연결", ["push", "pull", "clone", "푸시", "풀", "클론", "가져", "올리"]),
        ("프로젝트 협업에서 Git을 쓰는 이유", ["프로젝트", "협업", "관리", "팀", "코드"]),
    ]
    sections: list[dict[str, Any]] = []
    used: set[str] = set()
...
def build_youtube_content_summary(seed_url: str, title: str, payload: dict[str, Any], graph: dict[str, Any], collector_report: dict[str, Any]) -> str:
    ...
    sections = youtube_keyword_sections(title, transcript_text, limit=7) or youtube_chapter_sections(transcript_text, limit=7, title=title)
    core_problem = youtube_core_problem_from_profile(title, terms, "", transcript_text) if "youtube_core_problem_from_profile" in globals() else f"{title}의 핵심 개념을 문제 해결 기준으로 정리하는 것"
    step_rows = youtube_problem_steps(title, terms, sections, transcript_text) if "youtube_problem_steps" in globals() else []
    focus_items = terms[:12] or [section.get("title", "핵심 흐름") for section in sections[:8]]
    flow_items = [section.get("title", "핵심 흐름") for section in sections[:6]] or focus_items[:6]
    summary_rows = []
    for idx, item in enumerate(step_rows[:4], start=1):
        step_title, problem, cause, action, validation = item
        summary_rows.append(
            f"### {idx}. {step_title}\n"
            f"- 문제: {problem}\n"
            f"- 접근 방법: {cause}\n"
    ...
    if not summary_rows:
        for idx, item in enumerate(flow_items[:4], start=1):
            summary_rows.append(
                f"### {idx}. {item}\n"
                f"- 문제: 이 구간은 단순 자막 문장이 아니라 강의 전체에서 역할을 구분해야 하는 학습 단위다.\n"
                f"- 정리 방향: `{item}`를 정의, 필요성, 적용 상황, 확인 기준으로 나누어 정리한다.\n"
                f"- 확인 기준: 이 개념이 어떤 문제를 해결하는지 설명할 수 있는지 확인한다."
            )
...
## 강의 흐름 요약
{markdown_bullets(flow_items, "- 자막에서 충분한 주제 흐름을 만들지 못했습니다.")}

## 주요 내용 정리
{chr(10).join(summary_rows)}

## 정리 기준
이 요약은 자막 일부를 그대로 붙여넣는 방식이 아니라, 강의 제목과 자막에서 확인된 핵심 개념을 문제, 원인, 정리 방향, 확인 기준으로 재구성한 것이다. 자동 자막에 포함될 수 있는 인사말, 반복 표현, 문장 중간 절단은 본문 핵심 근거로 사용하지 않는다.
""")

def youtube_core_problem_from_profile(title: str, terms: list[str], user_problem: str, transcript_text: str = "") -> str:
    ...
    if profile == "copilot_cowork":
        return "클라우드 기반 AI 에이전트가 개발자의 작업 흐름을 어떻게 분해하고, 충돌과 검증 지점을 어떻게 관리해야 하는지 이해하는 것"
    if profile == "git_github":
        return "Git과 GitHub에서 변경 이력, 브랜치, 원격 저장소, 협업 흐름이 각각 어떤 문제를 해결하는지 구분하는 것"
    if profile == "data_structure_algorithm":
        return "자료구조와 알고리즘 개념을 정의 암기가 아니라 입력 조건, 처리 비용, 검증 기준으로 설명할 수 있게 만드는 것"
    focus = ", ".join(terms[:5]) if terms else "강의의 핵심 개념"
    return f"{title}에서 다루는 {focus}를 단순 개념 목록이 아니라 실제 학습자가 설명 가능한 문제 해결 기준으로 재구성하는 것"
...
def youtube_problem_steps(title: str, terms: list[str], sections: list[dict[str, Any]], transcript_text: str) -> list[tuple[str, str, str, str]]:
    ...
    if profile == "vector_retrieval":
        return [
            ...
            ("Semantic Search와 Hybrid Retrieval의 역할 분리", "의미 기반 검색은 관련성을 넓히는 데 유리하지만, 모든 검색 상황에서 키워드 조건을 완전히 대체한다고 보기 어렵다.", "고유명사, 코드, 제품명, 정확한 조건이 필요한 경우에는 키워드 기반 신호도 중요하다.", "Semantic Search는 의미적 유사도를 찾는 장치로, Hybrid Retrieval은 의미 신호와 키워드 신호를 함께 쓰는 전략으로 분리했다.", "확인 기준은 검색 결과가 의미적으로 넓어지는 동시에 정확히 포함되어야 하는 엔티티나 조건을 놓치지 않는지로 설정했다."),
            ("검색 품질을 결과 화면이 아니라 평가 기준으로 검증", "RAG나 검색 시스템은 결과가 그럴듯해 보여도 실제로는 관련 문서를 놓치거나 관련 없는 문서를 상위에 올릴 수 있다.", "따라서 ‘검색이 된다’와 ‘쓸 만한 검색 품질이 나온다’는 구분해야 한다.", "랭킹, 유사도, 검색 인덱스, 평가 지표를 하나의 검증 흐름으로 묶었다.", "검증 결과는 특정 쿼리에 대해 관련 문서가 상위에 등장하는지, 검색 결과가 답변 생성에 충분한 근거를 제공하는지로 판단한다."),
        ]
    if profile == "llm_engineering":
        return [
            ("LLM 학습 범위를 프롬프트 사용에서 AI Engineering 흐름으로 확장", "강의의 핵심 문제는 LLM을 단순히 호출하는 방법이 아니라 실제 애플리케이션으로 만들기 위해 필요한 구성 요소를 구분하는 데 있었다.", "RAG, 에이전트, 평가, 모니터링, 배포가 각각 따로 등장하면 전체 개발 흐름이 흐려질 수 있다.", "각 요소를 AI Engineering 파이프라인의 단계로 정리했다.", "검증 기준은 각 모듈이 어떤 실패를 막는지 설명할 수 있는지로 잡았다."),
            ("Course/Roadmap 정보를 개인 학습 계획으로 변환", "코스 소개 영상은 정보가 많지만 그대로 보면 일정, 모듈, 프로젝트, 과제 흐름이 섞여 핵심 학습 경로가 흐려질 수 있다.", "학습자가 실제로 필요한 것은 전체 코스 정보보다 다음에 무엇을 만들고 검증해야 하는지다.", "강의 안내를 학습 순서, 실습 단위, 질문 해결 경로, 프로젝트 산출물 기준으로 재구성했다.", "확인 기준은 다음에 무엇을 학습해야 하고 어떤 결과물을 만들어야 하는지 설명할 수 있는지다."),
            ("실무형 LLM 시스템의 검증 기준 분리", "LLM 앱은 동작하는 데모만으로 충분하지 않다.", "검색 품질, 답변 정확성, 운영 모니터링, 배포 안정성을 따로 보지 않으면 포트폴리오 설명이 피상적으로 남는다.", "실험 단계와 운영 단계의 확인 기준을 나누고 평가와 모니터링을 별도 문제 해결 단계로 정리했다.", "검증 기준은 결과물이 ‘AI 서비스를 어떻게 만들고 운영할지 이해했다’는 설명으로 이어지는지다."),
        ]
    if profile == "copilot_cowork":
        return [
            ("클라우드 기반 에이전트 작업 흐름 이해", "강의에서 중요한 지점은 에이전트가 로컬 도구처럼 즉시 실행되는 것만이 아니라 클라우드에서 계속 작업할 수 있다는 구조였다.", "작업이 계속 진행될수록 사용자가 언제 승인하고 언제 검토해야 하는지 구분하는 기준이 필요하다.", "클라우드 네이티브 실행, 모바일 확인, 작업 지속성을 하나의 사용자 경험 문제로 묶었다.", "검증 기준은 에이전트가 어떤 작업을 계속 수행하고 어느 지점에서 사람이 개입해야 하는지 구분하는 것이다."),
            ("개발 도구와 에이전트의 경계 분리", "VS Code, GitHub SDK, sandbox 같은 단어가 나오지만 핵심은 도구 목록이 아니라 에이전트가 개발 작업을 어떤 환경에서 안전하게 수행하는지다.", "도구명을 그대로 나열하면 강의의 중심 문제보다 키워드만 남을 수 있다.", "샌드박스 실행, 코드 변경, 충돌 확인, 결과 검증의 흐름으로 재정리했다.", "확인 기준은 에이전트가 만든 변경 사항을 사람이 어떤 기준으로 리뷰해야 하는지 설명할 수 있는지다."),
            ("충돌과 확장성을 운영 관점에서 보기", "에이전트가 많은 작업을 자동화할수록 일정 충돌, 코드 충돌, 권한, 플러그인 확장 같은 운영 문제가 함께 생긴다.", "자동화가 늘어날수록 검증과 통제 지점이 더 중요해진다.", "conflict, MCP, plugin, skill을 단순 키워드가 아니라 에이전트 운영 시 확인해야 할 제약으로 묶었다.", "검증 기준은 자동화가 늘어날 때 어떤 통제 지점이 필요한지 설명할 수 있는지다."),
        ]
    if profile == "git_github":
        return [
...
def youtube_learner_complex_experience(title: str, terms: list[str], sections: list[dict[str, Any]], transcript_text: str, core_problem: str) -> str:
    """Return learner-perspective complex problem-solving paragraphs for YouTube lecture notes.

    This section must describe the learner's conceptual/practical struggle with the lecture topic,
    not the app/AI's transcript processing or article generation process.
    """
    profile = youtube_topic_profile(title, transcript_text)
    if profile == "data_structure_algorithm":
        return """가장 복잡했던 부분은 자료구조와 알고리즘을 각각의 이름으로 외우는 것이 아니라, 문제 조건에 따라 선택 기준으로 바꾸는 일이었다. 시간 복잡도, 공간 복잡도, 빅오 표기법은 정의만 보면 단순한 성능 설명처럼 보이지만, 실제로는 입력 크기가 커질 때 어떤 풀이가 유지 가능하고 어떤 풀이가 금방 한계에 도달하는지를 판단하기 위한 기준이었다. 그래서 나는 이 개념들을 ‘암기해야 할 용어’가 아니라 ‘문제를 풀기 전에 비용을 예측하는 도구’로 다시 정리했다.
...
그래서 이번 학습에서는 임베딩을 ‘텍스트를 의미 공간의 벡터로 바꾸는 과정’으로, 벡터 데이터베이스를 ‘그 벡터를 저장하고 유사도 검색하는 계층’으로, hybrid retrieval을 ‘의미 신호와 키워드 신호를 함께 사용해 검색 품질을 보완하는 전략’으로 분리했다. 이 구분을 통해 RAG 시스템에서 검색 결과가 왜 답변 품질에 직접 영향을 주는지, 그리고 검색이 그럴듯해 보이는 것과 실제로 필요한 근거를 찾는 것은 다르다는 점을 이해할 수 있었다."""
    if profile == "llm_engineering":
        return """가장 복잡했던 부분은 LLM 학습을 프롬프트 사용법으로만 보지 않고, 실제 AI 애플리케이션을 만드는 엔지니어링 흐름으로 확장해서 이해하는 일이었다. 강의에서 다루는 RAG, 에이전트, 평가, 모니터링, 배포는 각각 따로 외우면 개념 목록처럼 보이지만, 실제 서비스에서는 하나의 파이프라인으로 이어진다. 검색이 부정확하면 답변 품질이 흔들리고, 평가 기준이 없으면 개선 여부를 판단할 수 없으며, 모니터링이 없으면 배포 이후의 품질 변화를 추적하기 어렵다.

따라서 이번 학습에서는 강의의 로드맵을 ‘무엇을 배운다’가 아니라 ‘어떤 실패를 줄이기 위해 어떤 구성 요소가 필요한가’로 다시 정리했다. RAG는 근거 부족 문제를 줄이기 위한 구조로, 에이전트는 도구 사용과 작업 분해 문제를 다루는 실행 흐름으로, 평가는 결과가 요구사항을 만족하는지 확인하는 기준으로, 모니터링은 운영 중 품질 변화를 추적하는 장치로 이해했다."""
    if profile == "copilot_cowork":
        return """가장 복잡했던 부분은 AI 에이전트를 단순 자동화 도구가 아니라, 사람이 맡기고 검토해야 하는 작업 흐름으로 이해하는 일이었다. 클라우드에서 계속 실행되는 에이전트는 사용자가 자리를 비워도 작업을 이어갈 수 있다는 장점이 있지만, 그만큼 어느 지점에서 사람이 승인하고 어떤 결과를 검토해야 하는지 기준이 필요하다. 자동화가 늘어날수록 편의성만 보는 것이 아니라 통제 지점, 충돌 가능성, 결과 검증 기준을 함께 봐야 했다.
...
def youtube_learner_outcome(title: str, terms: list[str], transcript_text: str, core_problem: str) -> str:
    profile = youtube_topic_profile(title, transcript_text)
    if profile == "data_structure_algorithm":
        return """이번 학습을 통해 자료구조와 알고리즘을 단순 정의가 아니라 문제 조건과 비용 기준으로 설명하는 연습을 할 수 있었다. 특히 시간 복잡도와 공간 복잡도는 시험용 용어가 아니라, 입력 규모가 커졌을 때 어떤 방식의 풀이가 더 적절한지 판단하는 기준으로 정리했다.

성과는 강의 내용을 ‘봤다’로 끝내지 않고, 배열·인덱스·탐색 같은 기본 개념을 접근 비용과 연산 비용 관점에서 다시 설명할 수 있게 된 점이다. 이후 알고리즘 문제를 풀 때도 먼저 입력 조건과 제한을 확인하고, 그 조건에서 어떤 자료구조와 알고리즘이 적합한지 설명하는 방식으로 복습할 수 있다."""
    if profile == "git_github":
        return """이번 학습을 통해 Git과 GitHub를 명령어 암기가 아니라 프로젝트 변경 이력을 안전하게 관리하고 협업 흐름을 만드는 도구로 이해할 수 있었다. commit은 변경을 기록하는 단위, branch는 작업을 분리하는 장치, merge는 검증된 변경을 합치는 과정, remote와 GitHub는 협업 기준점을 만드는 구조로 정리했다.

성과는 Git/GitHub 관련 용어를 개별 기능이 아니라 하나의 개발 workflow로 설명할 수 있게 된 점이다. 앞으로 포트폴리오 프로젝트를 관리할 때도 단순히 코드를 올리는 것이 아니라, 작업 단위를 branch로 나누고 commit 메시지로 변경 이유를 남기며, 원격 저장소를 통해 협업 가능한 흐름을 구성하는 기준으로 활용할 수 있다."""
    if profile == "vector_retrieval":
        return """이번 학습을 통해 벡터 데이터베이스와 임베딩을 단순한 검색 기술이 아니라 RAG 시스템의 근거 검색 품질을 좌우하는 핵심 구성 요소로 이해할 수 있었다. semantic search와 hybrid retrieval의 차이를 분리하면서, 검색 결과가 그럴듯하게 나오는 것과 실제 답변에 필요한 근거를 찾는 것은 다르다는 점을 확인했다.

성과는 검색 시스템을 구현할 때 단순히 벡터 DB를 붙이는 수준이 아니라, 어떤 검색 문제가 있고 그 문제를 임베딩·키워드 신호·랭킹·평가 기준으로 어떻게 나누어 해결해야 하는지 설명할 수 있게 된 점이다. 이후 RAG 프로젝트에서도 검색 품질을 별도 검증 대상으로 다룰 수 있는 기준을 만들었다."""
    if profile == "llm_engineering":
        return """이번 학습을 통해 LLM 애플리케이션 개발을 프롬프트 실험이 아니라 AI Engineering 흐름으로 이해할 수 있었다. RAG, 에이전트, 평가, 모니터링, 배포를 각각의 개념으로만 보는 것이 아니라 실제 서비스를 만들 때 필요한 단계와 검증 기준으로 연결했다.

성과는 강의 로드맵을 개인 학습 계획으로 바꿀 수 있게 된 점이다. 앞으로 LLM 프로젝트를 만들 때 단순 데모 구현에서 멈추지 않고, 검색 근거, 답변 평가, 운영 모니터링, 배포 안정성까지 포트폴리오 설명에 포함해야 한다는 기준을 세웠다."""
    if profile == "copilot_cowork":
        return """이번 학습을 통해 AI 에이전트를 단순 자동화 기능이 아니라 사람이 검토하고 통제해야 하는 작업 흐름으로 이해할 수 있었다. 클라우드 기반 실행, 샌드박스, 외부 시스템 연동, 충돌 확인은 모두 에이전트가 실제 업무에 들어올 때 필요한 신뢰 기준과 연결된다.

성과는 에이전트 기반 개발 도구를 볼 때 ‘무엇을 대신 해주는가’뿐 아니라 ‘어디서 검토해야 하고 어떤 실패를 막아야 하는가’를 함께 생각할 수 있게 된 점이다. 이는 AI Agent 포트폴리오를 설계할 때 기능 나열보다 작업 분해, 권한, 검증, 운영 제약을 설명하는 기준으로 이어진다."""
    focus = ", ".join(terms[:5]) if terms else "핵심 개념"
    return f"""이번 학습을 통해 {core_problem}이라는 문제를 중심으로 {focus}를 다시 설명할 수 있게 되었다. 단순히 강의를 들은 기록이 아니라, 어떤 개념이 왜 필요하고 어떤 기준으로 이해를 확인해야 하는지 학습자 관점에서 정리했다.

성과는 학습 내용을 포트폴리오에서 설명 가능한 단위로 바꾼 점이다. 앞으로 같은 주제를 다시 볼 때도 개념 이름을 나열하는 대신, 문제 상황, 원인, 조치, 확인 기준 순서로 복습할 수 있다."""


def youtube_concept_notes(title: str, terms: list[str], transcript_text: str) -> list[str]:
    descriptions = {
        "벡터 데이터베이스": "텍스트를 임베딩 벡터로 저장하고 유사도 검색을 수행하기 위한 저장·검색 계층으로 정리했다.",
        "임베딩": "문장이나 문서를 숫자 벡터로 바꾸어 의미적 거리를 계산할 수 있게 만드는 표현 방식으로 정리했다.",
        "의미 기반 검색": "단어가 정확히 일치하지 않아도 의미적으로 가까운 문서를 찾기 위한 검색 방식으로 이해했다.",
        "하이브리드 검색": "키워드 검색과 의미 기반 검색의 장점을 함께 사용해 검색 누락과 부정확한 랭킹을 줄이는 전략으로 정리했다.",
        "RAG": "검색된 근거를 LLM 답변 생성에 연결해 hallucination을 줄이고 답변 근거를 강화하는 구조로 정리했다.",
        "AI Engineering": "모델 호출을 넘어 데이터 수집, 검색, 평가, 배포, 운영까지 포함하는 서비스 구현 흐름으로 정리했다.",
        "에이전트": "목표를 수행하기 위해 도구를 호출하고 중간 상태를 판단하는 실행 단위로 정리했다.",
        "평가": "LLM 또는 검색 결과가 실제 요구사항을 만족하는지 확인하는 검증 단계로 정리했다.",
        "모니터링": "배포 이후 입력, 출력, 오류, 품질 변화를 추적하기 위한 운영 기준으로 정리했다.",
        "Git": "변경 이력을 추적하고 프로젝트 상태를 관리하기 위한 버전 관리 도구로 정리했다.",
        "GitHub": "원격 저장소, 리뷰, 협업 흐름을 연결하는 플랫폼으로 정리했다.",
        "commit": "변경 내용을 의미 있는 단위로 저장하는 기록 지점으로 정리했다.",
        "branch": "실험과 안정 작업을 분리하기 위한 작업 흐름으로 정리했다.",
        "시간 복잡도": "입력 크기가 커질 때 실행 시간이 어떻게 증가하는지 판단하는 기준으로 정리했다.",
        "공간 복잡도": "문제를 해결하기 위해 추가로 필요한 메모리 사용량을 판단하는 기준으로 정리했다.",
    }
    return [f"- **{term}**: {descriptions.get(term, '강의 흐름에서 독립 용어로 외우기보다 문제 상황, 역할, 확인 기준으로 나누어 정리해야 할 개념으로 다루었다.')}" for term in terms[:10]]




def youtube_learner_problem_sections(title: str, terms: list[str], transcript_text: str, core_problem: str) -> dict[str, str]:
    """Topic-focused learner voice sections for YouTube lecture notes.

    These sections must talk about the learner's difficulty with the lecture topic itself.
    They must not talk about transcript processing, article generation, AI, collectors, or writing mechanics.
    """
    profile = youtube_topic_profile(title, transcript_text)
...
특히 자료구조와 알고리즘은 문제를 풀고 난 뒤 결과만 맞추는 지식이 아니라, 풀이를 시작하기 전에 비용을 예측하고 선택을 설명하기 위한 기준이다. 그래서 이번 기록은 각 개념의 정의를 외우는 데서 멈추지 않고, 어떤 상황에서 해당 개념이 필요해지는지와 이해 여부를 어떻게 검증할 수 있는지 중심으로 정리했다.""",
            "problem_awareness": f"""이번 강의에서 가장 먼저 부딪힌 문제는 자료구조와 알고리즘 용어를 알고 있어도 실제 문제 상황에서 선택 기준으로 연결하기 어렵다는 점이었다. 시간 복잡도와 공간 복잡도는 각각 실행 시간과 메모리 사용량을 설명하는 개념처럼 보이지만, 실제로는 입력 크기가 커질 때 풀이가 감당 가능한지 미리 판단하기 위한 기준이다. 이 기준을 놓치면 예제에서는 동작하는 코드라도 데이터가 커졌을 때 왜 느려지는지 설명하기 어렵다.

특히 이번 영상의 중심 문제는 `{core_problem}`이었다. 배열, 인덱스, 탐색, 삽입, 삭제 같은 기본 개념도 이름만 외우면 단순한 자료형 설명으로 끝난다. 하지만 각 연산이 어떤 비용을 갖는지, 어떤 조건에서 장점과 한계가 드러나는지까지 연결해야 실제 알고리즘 문제나 프로젝트 코드에서 선택 이유를 설명할 수 있다.""",
...
앞으로 알고리즘 문제를 풀 때는 먼저 입력 크기, 필요한 연산, 메모리 제약을 확인하고 그 조건에 맞는 자료구조를 선택하는 흐름으로 복습할 수 있다. 이번 학습의 성과는 개념을 많이 외운 것이 아니라, 왜 특정 자료구조와 알고리즘이 필요한지 설명할 수 있는 기준을 만든 데 있다.""",
            "portfolio": """This learning record documents how I approached data structures and algorithms as decision-making tools rather than isolated definitions. I focused on time complexity, space complexity, arrays, indexing, and search as practical criteria for predicting cost before choosing an implementation.

The key outcome was building a clearer explanation framework: identify the input condition, estimate the operation cost, choose the appropriate data structure, and validate the decision. This makes the learning record useful not only as a course note, but also as evidence of technical reasoning for portfolio purposes.""",
            "skills": """- Explaining time complexity as input-size growth
- Explaining space complexity as memory-cost reasoning
- Connecting arrays and indexes to access cost
- Comparing search, insertion, and deletion trade-offs
- Choosing data structures based on problem conditions
- Separating definition memorization from implementation reasoning
- Building validation questions for algorithm learning
- Turning lecture concepts into technical decision criteria
- Communicating algorithmic reasoning in portfolio writing
- Avoiding unsupported code or formula invention""",
        }
    if profile == "git_github":
        return {
            "intro": f"""이번 학습에서는 `{title}` 강의를 보며 Git과 GitHub를 단순 명령어 모음이 아니라 프로젝트 변경 이력과 협업을 관리하는 흐름으로 이해하는 데 집중했다. 처음에는 commit, branch, merge, remote, push, pull이 각각 따로 떨어진 기능처럼 보이지만, 실제 프로젝트에서는 모두 변경 사항을 안전하게 기록하고 공유하기 위한 연결된 절차다.

그래서 이번 기록은 명령어를 나열하는 방식이 아니라, 각 기능이 어떤 개발 문제를 해결하는지 중심으로 정리했다. Git은 로컬 변경 이력을 관리하는 기준이고, GitHub는 원격 저장소와 리뷰 흐름을 통해 협업 가능한 상태를 만드는 기준으로 보았다.""",
            "problem_awareness": f"""이번 강의에서 가장 먼저 부딪힌 문제는 Git/GitHub를 명령어 사용법으로만 이해하면 실제 프로젝트 관리 상황에서 흐름이 쉽게 끊긴다는 점이었다. commit을 한다는 것은 단순 저장이 아니라 변경 이유와 시점을 기록하는 것이고, branch를 만든다는 것은 실험 작업과 안정적인 작업을 분리하는 것이다.

특히 이번 영상의 중심 문제는 `{core_problem}`이었다. 로컬 저장소와 원격 저장소, commit과 branch, merge와 conflict를 따로 외우면 협업 상황에서 왜 이런 절차가 필요한지 설명하기 어렵다. 따라서 각 명령이 막아주는 실패 상황과 협업에서의 역할을 함께 이해해야 했다.""",
...
앞으로 포트폴리오 프로젝트를 관리할 때도 단순히 코드를 올리는 것이 아니라, 작업 단위를 branch로 나누고 commit으로 변경 이유를 남기며 원격 저장소를 기준으로 협업 가능한 흐름을 구성하는 방식으로 적용할 수 있다.""",
            "portfolio": """This learning record frames Git and GitHub as a workflow for version control and collaboration rather than a list of commands. I focused on how commit, branch, merge, remote, push, and pull solve concrete problems in tracking, isolating, reviewing, and sharing changes.

The key outcome was building a practical explanation of Git/GitHub as a development habit: record meaningful changes, separate experiments from stable work, review before merging, and use the remote repository as a collaboration baseline.""",
            "skills": """- Explaining Git as version-control reasoning
- Using commit as a meaningful change record
- Understanding branch as work isolation
- Understanding merge as controlled integration
- Explaining remote repositories as collaboration baselines
- Connecting push and pull to team workflows
- Recognizing conflict as a workflow issue
- Translating Git commands into project-management decisions
- Communicating collaboration flow in portfolio writing
- Avoiding command-only explanations""",
        }
    if profile == "vector_retrieval":
        return {
            "intro": f"""이번 학습에서는 `{title}` 강의를 보며 벡터 데이터베이스와 임베딩을 단순 검색 기술이 아니라 RAG 시스템의 검색 품질을 결정하는 핵심 구조로 이해하는 데 집중했다. 키워드 검색, 의미 기반 검색, 하이브리드 검색은 각각 장단점이 다르기 때문에, 어떤 검색 문제가 있을 때 어떤 방식으로 보완해야 하는지 구분하는 것이 중요했다.

특히 임베딩은 텍스트를 숫자 벡터로 바꾸는 기술 설명에서 끝나지 않는다. 실제로는 질문과 문서가 같은 단어를 쓰지 않아도 의미적으로 가까운 내용을 찾기 위한 표현 방식이며, 벡터 데이터베이스는 그 표현을 저장하고 검색하는 계층으로 이해해야 한다.""",
            "problem_awareness": f"""이번 강의에서 가장 먼저 부딪힌 문제는 검색이 된다는 사실과 좋은 근거를 찾는다는 사실이 다르다는 점이었다. 키워드가 정확히 일치하면 검색은 쉬워 보이지만, 실제 사용자의 질문은 문서와 같은 표현을 쓰지 않는 경우가 많다. 반대로 의미 기반 검색만 사용하면 넓은 관련성은 찾을 수 있어도 정확한 조건이나 고유명사를 놓칠 수 있다.

특히 이번 영상의 중심 문제는 `{core_problem}`이었다. 벡터 데이터베이스, 임베딩, semantic search, hybrid retrieval을 각각 따로 외우면 RAG에서 왜 검색 품질이 답변 품질을 좌우하는지 설명하기 어렵다. 따라서 각 개념을 검색 실패를 줄이기 위한 역할로 구분해야 했다.""",
...
성과는 벡터 DB를 단순히 붙이는 기능으로 보지 않고, 검색 실패 유형과 평가 기준을 함께 고려해야 한다는 점을 학습한 것이다. 이후 RAG 프로젝트에서도 검색 결과의 관련성, 랭킹, 근거 충실도를 별도 검증 대상으로 다룰 수 있다.""",
            "portfolio": """This learning record frames vector databases, embeddings, semantic search, and hybrid retrieval as search-quality components in RAG systems. I focused on the practical problem of retrieving the right evidence when user queries and documents do not share the same wording.

The key outcome was learning to explain retrieval design as a set of trade-offs: semantic similarity improves recall across different expressions, keyword signals protect exact constraints, and hybrid retrieval combines both to improve grounding quality for downstream LLM answers.""",
            "skills": """- Explaining embeddings as semantic representations
- Understanding vector databases as similarity-search layers
- Distinguishing keyword search from semantic search
- Explaining hybrid retrieval trade-offs
- Connecting retrieval quality to RAG answer quality
- Identifying search failure modes
- Thinking about ranking and relevance validation
- Translating retrieval concepts into system-design reasoning
- Communicating RAG evidence flow in portfolio writing
- Avoiding buzzword-only explanations""",
        }
    # generic and other profiles use existing specialized complex/outcome but topic-first wording here
    return {
        "intro": f"""이번 학습에서는 `{title}` 강의를 보며 {term_text}를 단순한 개념 이름이 아니라 실제 문제를 설명하기 위한 기준으로 이해하는 데 집중했다. 강의에서 여러 개념이 순서대로 등장하더라도, 학습자로서는 각 개념이 어떤 상황에서 필요하고 어떤 문제를 줄이는지 구분할 수 있어야 한다.

그래서 이번 기록은 강의의 핵심 주제를 내가 다시 설명할 수 있는 기준으로 정리하는 데 초점을 두었다. 특히 {core_problem}이라는 문제를 중심에 두고, 관련 개념의 역할과 확인 기준을 분리했다.""",
        "problem_awareness": f"""이번 강의에서 가장 먼저 부딪힌 문제는 {term_text}가 각각 독립된 용어처럼 보이지만 실제로는 하나의 기술적 문제를 해결하기 위해 연결된다는 점이었다. 개념 이름을 알고 있어도 언제 사용해야 하는지, 어떤 한계를 보완하는지, 결과를 어떻게 확인해야 하는지 설명하지 못하면 학습이 실제 활용으로 이어지기 어렵다.

특히 이번 영상의 중심 문제는 `{core_problem}`이었다. 이 문제는 단순히 강의 내용을 따라가는 것으로 해결되지 않고, 핵심 개념을 역할별로 나누고 적용 조건을 정리해야 해결할 수 있었다.""",
        "problem_definition": f"""내가 정의한 문제는 {term_text}를 상황, 역할, 한계, 검증 기준으로 나누어 설명할 수 있게 만드는 것이었다. 강의에서 개념이 여러 개 등장할수록 중요한 것은 더 많은 용어를 기억하는 것이 아니라, 각 개념이 어떤 실패를 줄이고 어떤 판단을 가능하게 하는지 구분하는 일이다.

이를 위해 핵심 개념을 먼저 문제 상황과 연결했다. 어떤 조건에서 해당 개념이 필요해지는지, 비슷한 개념과 무엇이 다른지, 이해 여부를 어떤 질문으로 확인할 수 있는지를 기준으로 정리했다.""",
        "why": f"""이 문제를 중요하게 본 이유는 기술 학습에서 개념을 안다는 말이 실제로는 여러 수준으로 나뉘기 때문이다. 용어를 들어본 것, 정의를 말할 수 있는 것, 적용 상황을 구분할 수 있는 것, 결과를 검증할 수 있는 것은 서로 다르다. 이번 학습에서는 그중 적용 상황과 검증 기준까지 설명하는 것을 목표로 삼았다.

또한 실무나 포트폴리오에서는 단순히 강의를 수강했다는 사실보다, 특정 개념을 어떤 문제 해결 기준으로 이해했는지가 더 중요하다. 그래서 이번 강의의 핵심 개념을 문제 상황과 연결해 정리했다.""",
        "final": f"""이번 학습을 통해 {core_problem}이라는 문제를 중심으로 {term_text}를 다시 설명할 수 있게 되었다. 단순히 강의를 따라간 것이 아니라, 각 개념이 필요한 상황과 이해 여부를 확인하는 기준을 분리했다.

앞으로 같은 주제를 다시 학습할 때도 개념 이름을 나열하기보다, 문제 상황, 원인, 조치, 확인 기준 순서로 복습할 수 있다. 이 방식은 학습 내용을 실제 프로젝트 설명이나 기술 면접 답변으로 확장하는 데도 도움이 된다.""",
        "portfolio": f"""This learning record documents how I studied {term_text} as technical reasoning criteria rather than isolated terminology. I focused on identifying the core learning problem, separating related concepts by role, and defining how understanding could be validated.

The key outcome was turning lecture concepts into reusable explanation units: problem context, cause, action, and validation. This makes the note useful as portfolio evidence of structured technical learning.""",
        "skills": """- Identifying the core technical learning problem
- Separating related concepts by role and limitation
- Explaining when a concept should be applied
- Connecting lecture concepts to practical decision criteria
- Building validation questions for understanding
- Turning study notes into technical reasoning records
- Avoiding unsupported claims beyond the lecture topic
- Communicating learning outcomes in portfolio language
- Structuring problem, cause, action, and validation flow
...
def build_youtube_problem_medium_draft(
    ...
    steps = youtube_problem_steps(title, terms, sections, transcript_text)
    concept_notes = youtube_concept_notes(title, terms, transcript_text)
    quality = (collector_report.get("quality") if isinstance(collector_report.get("quality"), dict) else {}) or {}
    ...
    complex_experience_md = youtube_learner_complex_experience(title, terms, sections, transcript_text, core_problem)
    outcome_md = youtube_learner_outcome(title, terms, transcript_text, core_problem)
    learner_sections = youtube_learner_problem_sections(title, terms, transcript_text, core_problem)
    intro_md = learner_sections.get("intro", "")
    problem_awareness_md = learner_sections.get("problem_awareness", "")
    problem_definition_md = learner_sections.get("problem_definition", "")
    why_problem_md = learner_sections.get("why", "")
    final_md = learner_sections.get("final", "")
    portfolio_md = learner_sections.get("portfolio", "")
    skills_md = learner_sections.get("skills", "")

    return sanitize_medium_markdown(f"""# {title}: 학습 문제를 해결하며 핵심 개념 정리하기

_A learner-centered Medium portfolio note on technical concept understanding_
...
## 복잡한 문제 해결 경험
{complex_experience_md}

## 성과
{outcome_md}

## 사용한 주요 수식/코드 정리
이번 강의에서 그대로 옮겨 적을 수 있는 코드 또는 수식 원문은 명확하게 확인되지 않았다. 그래서 임의의 코드를 만들기보다, 강의에서 다룬 개념을 실제 문제 해결 기준으로 정리했다.

{concept_md}

코드나 수식이 직접 제시되지 않은 경우에도 학습 성과를 남길 수 있다. 이때 중요한 것은 개념을 단순 정의로 끝내지 않고, 어떤 문제 상황에서 필요하고 어떤 기준으로 이해를 확인할 수 있는지 설명하는 것이다. 이후 같은 주제를 실습 코드로 구현하게 되면 해당 코드와 검증 결과를 이 섹션에 추가할 수 있다.
...
## 이미지 번호와 캡션 목록
- 이번 입력에는 이미지가 제공되지 않았으므로 별도 이미지 캡션은 작성하지 않았다.
""")
...
def youtube_topic_plan(text: str) -> list[tuple[str, list[str]]]:
...
def infer_learning_terms(text: str, limit: int = 16) -> list[str]:
    ...
    lower = source.lower()
    title_hint = source.splitlines()[0].lower() if source.splitlines() else lower[:180]
    profile = youtube_topic_profile(title_hint, source) if "youtube_topic_profile" in globals() else "general_lecture"
    if profile != "general_lecture" and "youtube_domain_terms" in globals():
        return youtube_domain_terms(title_hint, source, limit=limit)
...
def should_use_source_graph_direct_article(seed_url: str, collector_report: dict[str, Any], source_pack_text: str) -> bool:
    ...
    if "wikidocs.net" in host and stats["chars"] >= 3000:
        return True
    if "oopy.io" in host and stats["chars"] >= 3000:
        return True
    if "youtube.com" in host or "youtu.be" in host:
...
def is_meaningful_problem_text(text: str) -> bool:
    """Return True only when optional user problem text contains a real problem sentence."""
    raw = str(text or "")
    normalized = re.sub(r"\s+", " ", raw).strip()
    if not normalized:
        return False
    lowered = normalized.lower()
    empty_values = {"없음", "없습니다", "n/a", "na", "none", "null", "undefined", "-", ".", "..", "..."}
    if lowered in empty_values:
        return False
    if "생성 직전 사용자가 정의한 어려운 문제" in normalized:
        return False
    if "자료의 핵심 흐름을 바탕으로 문제를 정의" in normalized:
        return False
    # Reject punctuation-only residue such as '.', '` . `', ':' left after placeholder removal.
    alnum = re.sub(r"[^0-9A-Za-z가-힣]", "", normalized)
    if len(alnum) < 4:
        return False
    return True
...
def clean_prompt_memo(memo: str) -> str:
    """Remove UI helper labels/placeholders from the user's optional problem memo."""
    text = str(memo or "")
    placeholder_patterns = [
        r"\[생성 직전 사용자가 정의한 어려운 문제\]",
        r"\[생성 직전 사용자가 적은 어려움/헷갈린 부분\]",
        r"없음\.\s*자료의 핵심 흐름을 바탕으로 문제를 정의하고 해결 과정 작성\.??",
        r"사용자가 정의한 어려운 문제",
    ]
    ...
    for line in text.splitlines():
        ...
        if not stripped:
            continue
        if not is_meaningful_problem_text(stripped):
            continue
        cleaned_lines.append(stripped)
    cleaned = "\n".join(cleaned_lines).strip()
    return cleaned if is_meaningful_problem_text(cleaned) else ""
...
class Handler(BaseHTTPRequestHandler):
    ...
    def create_direct_blog(self) -> None:
        ...
        url_run_id = make_generation_run_id() if url_only_mode else ""
        seed_url = ""
        if url_only_mode:
            urls = extract_urls_from_text(raw_text)
            seed_url = normalize_seed_url_for_collection(urls[-1]) if urls else ""  # use the most recently pasted URL when multiple URLs remain in the textarea
            ...
            if source_pack_text.strip():
                ...
                high_value_hosts = ("aiskillsnavigator.microsoft.com", "oopy.io", "youtube.com", "youtu.be", "wikidocs.net")
                if any(h in url_domain(seed_url) for h in high_value_hosts) and collector_report.get("error"):
                    result = {
        ...
        if isinstance(result, dict):
            ...
            if url_only_mode:
                ...
                ok, reason = article_matches_seed_url(seed_url, draft_text)
                if not ok:
                    ...
                    return self.json(result)
            result.update({"elapsed_seconds": elapsed, "image_count": len(image_files), "mode": "url_only_source_graph" if url_only_mode else "batch_upload"})
            return self.json(result)
        if url_only_mode:
            ok, reason = article_matches_seed_url(seed_url, str(result))
            if not ok:
                return self.json({
    ...
    def debug_collect_url(self) -> None:
        data = self.read_json()
        seed_url = normalize_seed_url_for_collection(str(data.get("url") or "").strip())
        if not seed_url:
...
async function makeBlog(formatType) {
  ...
  const extraInfo = [
    ...
    ["생성 직전 사용자가 정의한 어려운 문제", isLectureSummary ? "" : (promptNote || "없음. 자료의 핵심 흐름을 바탕으로 문제를 정의하고 해결 과정 작성")]
  ]
