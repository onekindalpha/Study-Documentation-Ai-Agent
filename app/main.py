...
def collector_source_graph(collector_report: dict[str, Any], max_nodes: int = 30) -> dict[str, Any]:
    ...
    return {
        ...
        "video_url_candidates": payload.get("video_url_candidates", [])[:16] if isinstance(payload.get("video_url_candidates"), list) else [],
...
""")
    ...
    if "aiskillsnavigator.microsoft.com" in host and is_purview_security_topic:
        ...
        article_title = "CS 핵심요약 학습: 면접 답변으로 설명 가능한 단위로 다시 나누기"
        subtitle = "Turning CS summary pages into interview-ready explanation blocks"
        learning_scope = "운영체제, 네트워크, 자료구조, 데이터베이스 등 CS 핵심 개념"
        learning_flow_label = "개념 정의 → 동작 원리 → 왜 필요한가 → 대표 질문 → 꼬리 질문 대비"
        core_problem = "처음 헷갈린 지점은 운영체제, 네트워크, 자료구조 같은 CS 개념을 많이 읽는 것과 면접에서 구조적으로 설명하는 것이 다르다는 점이었다."
        result_summary = "CS 요약 내용을 단순 암기 목록이 아니라 정의·원리·예시·비교 기준으로 말할 수 있는 답변 단위로 재정리했다."
        evidence_flow = headings[:14] or ["운영체제", "네트워크", "자료구조", "데이터베이스", "CS"]
        lab_evidence = []
        concepts = [
        ...
        step_sections = [
            ...
            ("암기와 설명의 차이 분리", "짧은 정의만 외우면 실제 질문에서 왜 중요한지 설명하기 어렵다.", "개념마다 무엇인가, 왜 필요한가, 어떤 문제가 생기는가, 어떻게 해결하는가 구조로 바꾸었다.", "최종 글이 용어 목록이 아니라 답변 구조를 갖는지 확인했다."),
        ]
        portfolio_skills = ["CS 개념 구조화", "기술 면접 답변 구성", "개념 비교", "복습 기준 설정"]
    else:
...
4. bind entity types to data와 preview ontology가 실습 완료 기준이 되는 이유"""
        ...
        problem_definition_md = """이 학습에서 문제로 잡은 것은 CS 개념을 많이 읽는 것과 면접에서 설명 가능한 구조로 말하는 것이 다르다는 점이었다. 핵심은 운영체제, 네트워크, 자료구조 같은 개념을 정의만 외우는 것이 아니라 원리, 필요성, 대표 질문, 꼬리 질문으로 나누어 설명할 수 있게 만드는 것이었다."""
        why_problem_md = """이 내용을 문제로 본 이유는 CS 요약 자료를 그대로 읽으면 용어는 익숙해져도 면접 질문을 받았을 때 논리적으로 설명하기 어렵기 때문이다. 기술 면접에서는 단순 정의보다 왜 필요한지, 어떤 문제가 생기는지, 비슷한 개념과 무엇이 다른지를 설명해야 한다."""
        complex_summary_md = """가장 복잡했던 부분은 넓은 CS 범위를 암기 목록이 아니라 답변 구조로 바꾸는 것이었다. 운영체제 개념은 실행 환경과 자원 관리 관점으로, 네트워크는 요청과 응답이 이동하는 계층 구조로, 자료구조는 저장과 접근 방식이 성능에 미치는 영향으로 정리했다."""
        final_summary_md = """이번 학습을 통해 CS 핵심요약 자료를 면접 답변 단위로 다시 나누었다. 단순히 많이 읽는 것이 아니라, 각 개념을 정의·원리·예시·비교 기준으로 설명할 수 있게 만드는 것이 핵심이었다."""
    else:
...
def should_use_source_graph_direct_article(seed_url: str, collector_report: dict[str, Any], source_pack_text: str) -> bool:
    ...
    if "oopy.io" in host and stats["chars"] >= 3000:
        return True
    if ("youtube.com" in host or "youtu.be" in host) and ("[영상 자막]" in source_pack_text or "[영상 설명]" in source_pack_text):
        return True
    return False
...
def clean_prompt_memo(memo: str) -> str:
    ...
    for line in (memo or "").splitlines():
        ...
        if not stripped:
            continue
        if stripped.startswith("[생성 직전 사용자가 적은 어려움/헷갈린 부분]"):
            continue
